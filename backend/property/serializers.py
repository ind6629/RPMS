...
class RepairOrderCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_property(self, value):
        ...
        if user and getattr(user, 'role', None) == 'owner' and value.owner_id != user.id:
...
class ServiceFeedbackCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_order(self, order):
        ...
        user = getattr(request, 'user', None)
        visible_to_owner = (
            order.user_id == getattr(user, 'id', None)
            or order.property.owner_id == getattr(user, 'id', None)
        )
        if not user or not visible_to_owner:
            raise serializers.ValidationError('仅报修人或房产业主可评价')
